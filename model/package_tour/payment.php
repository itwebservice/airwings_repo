...
///////////////////////////////////***** Package Tour payment master save start *********/////////////////////////////////////////////////////////////
 public function package_tour_payment_master_save()
 {
      ...
      $sq= mysql_query(" insert into package_payment_master (payment_id, booking_id, financial_year_id, branch_admin_id, emp_id, date, payment_mode, amount, bank_name, transaction_id, payment_for, travel_type, bank_id, clearance_status ) values ('$max_payment_id', '$booking_id', '$financial_year_id', '$branch_admin_id', '$emp_id', '$payment_date', '$payment_mode', '$payment_amount', '$bank_name', '$transaction_id', '$payment_for', '$p_travel_type', '$bank_id', '$clearance_status') ");
 ...
 public function payment_finance_save($payment_id)
 {
    ...
    $payment_particular = get_sales_paid_particular(get_package_booking_payment_id($booking_id,$yr1), $payment_date, $payment_amount1, $customer_id, $payment_mode, get_package_booking_id($booking_id,$yr1));
    ...
    $payment_amount = $payment_amount1;
    $payment_date = $payment_date;
    $payment_particular = get_sales_particular(get_package_booking_id($booking_id,$yr1), $payment_date, $payment_amount1, $customer_id);
    $ledger_particular = get_ledger_particular('By','Cash/Bank');
    $gl_id = $cust_gl;
    $payment_side = "Credit";
    $clearance_status = "";
 ...
 public function bank_cash_book_save($payment_id)
{
  ...
  $bank_id = $bank_id;
  $particular = get_sales_paid_particular(get_package_booking_payment_id($payment_id,$yr1), $payment_date, $payment_amount, $sq_booking_info['customer_id'], $payment_mode, get_package_booking_id($booking_id,$yr1));
  $clearance_status = ($payment_mode=="Cheque") ? "Pending" : "";
  $payment_side = "Debit";
...
function finance_update($sq_payment_info, $clearance_status1)
{
    ...
    if($payment_amount1 > $payment_old_value)
    {
        ...
        $payment_date = $payment_date;
        $payment_particular = get_sales_particular(get_package_booking_id($booking_id,$yr1), $payment_date, $balance_amount, $customer_id);
        $ledger_particular = get_ledger_particular('By','Cash/Bank');
        $gl_id = $cust_gl;
        $payment_side = "Credit";
        $clearance_status = "";
        ...
        $payment_date = $payment_date;
        $payment_particular = get_sales_paid_particular(get_package_booking_payment_id($booking_id,$yr1), $payment_date, $payment_old_value, $customer_id, $payment_mode, get_package_booking_id($booking_id,$yr1));
        $ledger_particular = get_ledger_particular('By','Cash/Bank');
        $gl_id = $pay_gl;
        $payment_side = "Credit";
        $clearance_status = ($payment_mode!="Cash") ? "Pending" : "";
...
///////////////////////////////***** Package Tour Payment Master update end*******///////////////////////////////////////////////////////////// 
 ...
 {
   $sq_total_paid = mysql_fetch_assoc(mysql_query("select sum(amount) as sum from package_payment_master where booking_id='$booking_id'"));
   ...
   global $model;
   $model->generic_payment_mail('45',$payment_amount, $payment_mode, $total_amount, $paid_amount, $payment_date, $email_id, $subject);
 }
...
//////////////////////////////////**Payment email notification send start**/////////////////////////////////////
 public function payment_update_email_notification_send($payment_id)
 {
   ...
   global $model;
   $model->generic_payment_mail('56',$payment_amount, $payment_mode, $total_amount, $paid_amount, $payment_date, $email_id,$subject, $update_payment);
 }
//////////////////////////////////**Payment email notification send end**/////////////////////////////////////
 ...
 public function payment_sms_notification_send($booking_id, $payment_amount, $payment_mode,$customer_id)
 {
  ...
  $fname = $sq_customer['first_name'];
   $lname = $sq_customer['last_name'];
    ...
    $message = "Dear ". $fname." ". $lname.", We have received a payment of Rs ".$payment_amount." for your upcoming Trip with ".$app_name.". Wish you a happy Journey";
    global $model;
