...
public function location_master_save()
{
    ...
    $location_name1 = ltrim($location_name);
	$location_count = mysql_num_rows( mysql_query("select * from locations where location_name='$location_name1'") );
	if($location_count>0){
...
public function location_master_update()
{
	...
	$location_count = mysql_num_rows( mysql_query("select * from locations where location_name='$location_name1' and location_id!='$location_id'") );
	if($location_count>0){
		echo "error--Sorry, Location name already exists.";
		exit;
	}
