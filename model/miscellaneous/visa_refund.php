...
public function finance_save($refund_id)
{
    ...
    $payment_particular = get_refund_paid_particular(get_misc_booking_id($misc_id,$yr), $refund_date, $refund_amount, $refund_mode,get_misc_booking_refund_id($refund_id,$yr1));
    $ledger_particular = '';
    $gl_id = $pay_gl;
    $payment_side = "Credit";
    $clearance_status = "";
...
public function bank_cash_book_save($refund_id)
{
	...
	$transaction_id = $transaction_id;
	$bank_id = $bank_id;
	$particular = get_refund_paid_particular(get_misc_booking_id($misc_id,$yr), $refund_date, $refund_amount, $refund_mode, get_misc_booking_refund_id($refund_id,$yr1));
      ...
      <table cellspacing="0" style="width:100%">
	      ...
	      <tr>
	        <td style="background: rgba(255, 255, 255, 0.23);color: #000;">
	          <p style="line-height: 24px;padding: 0 0 0 15px;">Booking ID : '.$sq_sq_visa_info['misc_id'].'</p>
...
$subject = "Miscellaneous Cancellation Refund ( ".get_misc_booking_id($misc_id,$year)." )";
  ...
  $model->app_email_send('100',$cust_email['email_id'], $content,$subject);
}
