...
public function ticket_payment_master_save()
{
	...
	$sq_payment = mysql_query("insert into ticket_payment_master (payment_id, branch_admin_id, ticket_id, financial_year_id, payment_date, payment_amount, payment_mode, bank_name, transaction_id, bank_id, clearance_status) values ('$payment_id', '$branch_admin_id', '$ticket_id', '$financial_year_id', '$payment_date', '$payment_amount', '$payment_mode', '$bank_name', '$transaction_id', '$bank_id', '$clearance_status') ");
	if(!$sq_payment){
		rollback_t();
		...
		if($GLOBALS['flag']){
			...
			echo "Ticket payment saved successfully!";
			exit;	
		}
		else{
			rollback_t();
...
public function finance_save($payment_id, $branch_admin_id)
{
    ...
    $payment_particular = get_sales_paid_particular(get_ticket_booking_payment_id($ticket_id,$yr1), $payment_date, $payment_amount1, $customer_id, $payment_mode, get_ticket_booking_id($ticket_id,$yr1));
    ...
    $payment_amount = $payment_amount1;
    $payment_date = $payment_date;
    $payment_particular = get_sales_particular(get_ticket_booking_id($ticket_id,$yr1), $payment_date, $payment_amount1, $customer_id);
    $ledger_particular = get_ledger_particular('By','Cash/Bank');
    $gl_id = $cust_gl;
    $payment_side = "Credit";
    $clearance_status = "";
...
public function bank_cash_book_save($payment_id, $branch_admin_id)
{
	...
	$sq_ticket_info = mysql_fetch_assoc(mysql_query("select customer_id from ticket_master where ticket_id='$ticket_id'"));
	...
	$bank_id = $bank_id;
	$particular = get_sales_paid_particular(get_ticket_booking_payment_id($payment_id,$yr1), $payment_date, $payment_amount, $sq_ticket_info['customer_id'], $payment_mode, get_ticket_booking_id($ticket_id,$yr1));
...
public function ticket_payment_master_update()
{
	...
	$sq_payment = mysql_query("update ticket_payment_master set ticket_id='$ticket_id', financial_year_id='$financial_year_id', payment_date='$payment_date', payment_amount='$payment_amount', payment_mode='$payment_mode', bank_name='$bank_name', transaction_id='$transaction_id', bank_id='$bank_id', clearance_status='$clearance_status' where payment_id='$payment_id' ");
	if(!$sq_payment){
		...
		if($GLOBALS['flag']){
			...
			echo "Ticket payment update successfully!";
			exit;	
		}
		else{
			rollback_t();
...
public function finance_update($sq_payment_info, $clearance_status1)
{
	...
	if($payment_amount1 > $payment_old_value)
	{
	    ...
	    $payment_date = $payment_date;
	    $payment_particular = get_sales_particular(get_ticket_booking_id($ticket_id,$yr1), $payment_date, $balance_amount, $customer_id);
			$ledger_particular = get_ledger_particular('By','Cash/Bank');
	    $gl_id = $cust_gl;
	    $payment_side = "Credit";
	    $clearance_status = "";
	    ...
	    $payment_date = $payment_date;
	    $payment_particular = get_sales_paid_particular(get_ticket_booking_payment_id($ticket_id,$yr1), $payment_date, $payment_old_value, $customer_id, $payment_mode, get_ticket_booking_id($ticket_id,$yr1));
			$ledger_particular = get_ledger_particular('By','Cash/Bank');
	    $gl_id = $pay_gl;
	    $payment_side = "Credit";
	    $clearance_status = ($payment_mode!="Cash") ? "Pending" : "";
...
public function payment_email_notification_send($ticket_id, $payment_amount, $payment_mode, $payment_date)
{
   ...
   $sq_customer_info = mysql_fetch_assoc(mysql_query("select email_id from customer_master where customer_id='$sq_ticket_info[customer_id]'"));
   $email_id = $sq_customer_info['email_id'];

   $sq_total_amount = mysql_fetch_assoc(mysql_query("select sum(payment_amount) as sum from ticket_payment_master where ticket_id='$ticket_id'"));
   ...
   global $model;
   $model->generic_payment_mail('48',$payment_amount, $payment_mode, $total_amount, $paid_amount, $payment_date, $email_id,$subject, $payment_id);
}
...
public function payment_update_email_notification_send($payment_id)
{
	...
	global $model;
   	$model->generic_payment_mail('58',$payment_amount, $payment_mode, $total_amount, $paid_amount, $payment_date, $email_id,$subject, $update_payment);
...
public function payment_sms_notification_send($ticket_id, $payment_amount, $payment_mode)
{
	...
	$message = "Acknowledge your payment of Rs. ".$payment_amount.", ".$payment_mode." which we received for Flight Ticket installment.";
    global $model;
