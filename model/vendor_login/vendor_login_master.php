...
public function vendor_login_save($username, $password,$vendor_type, $user_id, $active_flag, $email_id,$opening_balance,$side,$as_on_date){
  ...
  $sq_login = mysql_query("insert into vendor_login (login_id, username, password,email, vendor_type, user_id, active_flag) values ('$login_id', '$username', '$password', '$email_id', '$vendor_type', '$user_id', '$active_flag')");
...
public function vendor_login_update($login_id, $username, $password, $user_id, $active_flag, $email_id,$vendor_type, $opening_balance,$side1,$as_of_date)
{
  ...
  $q = "update ledger_master set balance='0', ledger_name='$ledger_name', dr_cr='$side1' where user_type='$vendor_type' and customer_id='$user_id' ";
...
$model->app_email_send('3',$email_id, $content,$subject,'1');
...
{
   global $app_name,$app_website;


 $message = "Dear ".$username.", Thanks for join ".$app_name.". We wish a great journey with you. L : ".BASE_URL."/view/vendor_login/"." "."  U : ".$username."  P : ".$password." For more details login ".$app_website." ";
