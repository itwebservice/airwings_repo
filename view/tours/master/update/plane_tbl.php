...
<div class="row mg_bt_10">
    <div class="col-md-12">
        ...
        <table id="tbl_package_tour_quotation_dynamic_plane_update" name="tbl_package_tour_quotation_dynamic_plane_update" class="table table-bordered no-marg pd_bt_51">
			<?php 
			...
			if($sq_plane_count==0){
				...
				<tr>
		            ...
		            <td><input type="text" id="txt_dapart-1"  name="txt_dapart-1" class="app_datetimepicker" placeholder="Departure Date & Time" title="Departure Date & Time" style="width: 100px;" onchange="get_to_datetime(this.id,'txt_arrval-1')"  value="<?= date('d-m-Y H:i:s') ?>" /></td>
				...
				while($row_q_plane = mysql_fetch_assoc($sq_q_plane)){
		            ...
		            <script>
		            	...
		            	$('#plane_from_location-<?= $offset.$count ?>_d, #plane_to_location-<?= $offset.$count ?>_d, #airline_name-<?= $offset.$count ?>_d,#from_city-1').select2();
		            </script>
					<?php
				}
			}
