	                    ...
	                    <table id="tbl_dynamic_passport_update" name="tbl_dynamic_passport_update" class="table table-bordered no-marg" style="width:1100px">
	                       ...
	                       if($sq_entry_count==0){
	                       		...
	                       		while($row_entry = mysql_fetch_assoc($sq_entry)){
	                       			...
	                       			?>
									 <tr>
									    <td><input class="css-checkbox" id="chk_passport<?= $offset.$count ?>_d" type="checkbox" checked disabled><label class="css-label" for="chk_passport<?= $offset ?>" > <label></td>
									    ...
									    <td style="width:80px"><input type="text" id="adolescence<?= $offset.$count ?>_d" name="adolescence<?= $offset.$count ?>_d" placeholder="Adolescence" title="Adolescence" disabled value="<?= $row_entry['adolescence'] ?>" /></td>
									...
									<script>
									$("#birth_date<?= $offset.$count ?>_d").datetimepicker({ timepicker:false, format:'d-m-Y' });
									</script>    
	                       			<?php
	        ...
	        <div class="panel panel-default panel-body fieldset mg_tp_30">
	        	...
	        	<div class="row">
		        	<div class="col-md-3 col-sm-6 col-xs-12 mg_bt_10">
		        		<input type="text" id="passport_issue_amount1" name="passport_issue_amount1" placeholder="Amount" title="Amount" value="<?= $sq_passport_info['passport_issue_amount'] ?>" onchange="calculate_total_amount('1');validate_balance(this.id)">
		        	</div>
		        	<div class="col-md-3 col-sm-6 col-xs-12 mg_bt_10">
		        		<input type="text" name="service_charge1" id="service_charge1" placeholder="Service Charge" title="Service Charge" value="<?= $sq_passport_info['service_charge'] ?>" onchange="calculate_total_amount('1');validate_balance(this.id)">
		        	</div>		
		        	<div class="col-md-3 col-sm-6 col-xs-12 mg_bt_10">
	                  <select name="taxation_type1" id="taxation_type1" title="Taxation Type">
	                    <option value="<?= $sq_passport_info['taxation_type'] ?>"><?= $sq_passport_info['taxation_type'] ?></option>
		        	...
		        	<div class="col-md-3 col-sm-6 col-xs-12 mg_bt_10">
	        			<select name="taxation_id1" id="taxation_id1" title="Tax" onchange="generic_tax_reflect(this.id, 'service_tax1', 'calculate_total_amount', '1');">
	                       <?php 
	                       if($sq_passport_info['taxation_id']!='0'){
	                         $sq_taxation = mysql_fetch_assoc(mysql_query("select * from taxation_master where taxation_id='$sq_passport_info[taxation_id]'"));
				        ...
				        <input type="hidden" id="service_tax1" name="service_tax1" value="<?= $sq_passport_info['service_tax'] ?>">
	        		</div>	   	   		
		        </div>
		        <div class="row">	        		
	        		<div class="col-md-3 col-sm-6 col-xs-12 mg_bt_10_xs">
	        		...
	        		<div class="col-md-3 col-sm-6 col-xs-12">
	        			<input type="text" name="balance_date1" id="balance_date1" placeholder="Booking Date" title="Booking Date" value="<?= get_date_user($sq_passport_info['created_at']) ?>" onchange="check_valid_date(this.id)">
	        		</div>
	        	</div>
	        </div> 
...
$('#birth_date_u1,#due_date1,#balance_date1').datetimepicker({ timepicker:false, format:'d-m-Y' });
			...
			$.post(base_url+'view/load_data/finance_date_validation.php', { check_date: check_date1 }, function(data){
				if(data !== 'valid'){
					...
					$.ajax({
						...
						data:{ passport_id : passport_id, customer_id : customer_id, passport_issue_amount : passport_issue_amount, service_charge : service_charge, taxation_type : taxation_type, taxation_id : taxation_id, service_tax : service_tax, service_tax_subtotal : service_tax_subtotal, passport_total_cost : passport_total_cost, honorific_arr : honorific_arr, first_name_arr : first_name_arr, middle_name_arr : middle_name_arr, last_name_arr : last_name_arr, birth_date_arr : birth_date_arr, adolescence_arr : adolescence_arr, received_documents_arr : received_documents_arr, entry_id_arr : entry_id_arr, due_date1 : due_date1,balance_date1 : balance_date1 },
