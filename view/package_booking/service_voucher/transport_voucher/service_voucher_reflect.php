...
$sq_quotation = mysql_fetch_assoc( mysql_query("select * from package_tour_quotation_master where quotation_id='$sq_booking[quotation_id]'") );
	...
	while($row_entry1 = mysql_fetch_assoc($sq_entry_n)){
		...
		<div class="row">
			<div class="col-md-4 col-sm-6 mg_bt_10">
			    <h3 class="editor_title">Special Note</h3>
				<textarea id="special_arrangments" class="feature_editor" name="special_arrangments" placeholder="Special Note" title="Special Note"><?= $sq['special_arrangments'] ?></textarea>
			</div>
			<div class="col-md-4 col-sm-6 mg_bt_10">
			    		    <h3 class="editor_title">Inclusions</h3>
				<textarea id="inclusions" name="inclusions" class="feature_editor" placeholder="Inclusions"  title="Inclusions"><?= $sq_quotation['inclusions'] ?></textarea>
			</div>
			<div class="col-md-4 col-sm-6 mg_bt_10">
			    		    <h3 class="editor_title">Exclusions</h3>
				<textarea id="exclusion" name="exclusion" class="feature_editor" placeholder="Exclusion" title="Exclusion"><?= $sq_quotation['exclusions'] ?></textarea>
			</div>
		</div>
			...
			submitHandler:function(form){
					...
					var exclusion = $('#exclusion').val();
								...
								$.ajax({
									...
									data:{ booking_id : booking_id,vehicle_name_array:vehicle_name_array,  pick_array : pick_array, drop_to_array : drop_to_array,driver_name_array : driver_name_array,driver_contact_array : driver_contact_array, special_arrangments : special_arrangments, confirm_by : confirm_by, inclusions : inclusions ,exclusion:exclusion},
				                ...
				                var msg = message.split('--');
									...
									else
									{
							        	...
							        	if(data1=="yes"){
													var url1 = base_url+'model/app_settings/print_html/voucher_html/transport_voucher.php?booking_id='+booking_id;
													loadOtherPage(url1);
									    }
									}
								});
