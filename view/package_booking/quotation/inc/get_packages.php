              ...
              <div id="collapse_<?= $count?>" class="panel-collapse collapse main_block" role="tabpanel" aria-labelledby="heading_<?= $count?>">
                <div class="panel-body">
                  <div class="col-md-12 no-pad" id="div_list1">
                    ...
                    <table style="width:100%" id="dynamic_table_list_p_<?= $row_tours['package_id'] ?>" name="dynamic_table_list_p_<?= $row_tours['package_id'] ?>" class="table mg_bt_0 table-bordered">
                    <legend>Tour Itinerary</legend>
                      ...
                      ?>
                       <tr>
                          ...
                          <td width="50px;"><input maxlength="15" value="<?= $offset1 ?>" type="text" name="username" placeholder="Sr. No." class="form-control mg_bt_10" disabled /></td>
                          <td class='col-md-3'><input type="text" id="special_attaraction<?php echo $offset; ?>" onchange="validate_spaces(this.id);validate_spattration(this.id);" name="special_attaraction" class="form-control mg_bt_10" placeholder="Special Attraction" title="Special Attraction"  value="<?php echo $row_program['attraction']; ?>"></td>
                          <td class='col-md-5' style="max-width: 594px;overflow: hidden;"><textarea id="day_program<?php echo $offset; ?>" name="day_program" class="form-control mg_bt_10" title="Day-wise Program" rows="3" placeholder="*Day-wise Program" onchange="validate_spaces(this.id);validate_dayprogram(this.id);" value="<?php echo $row_program['day_wise_program']; ?>"><?php echo $row_program['day_wise_program']; ?></textarea></td>
                          <td class='col-md-2' style='width:100px'><input type="text" id="overnight_stay<?php echo $offset; ?>" name="overnight_stay" onchange="validate_spaces(this.id);validate_onstay(this.id);" class="form-control mg_bt_10" placeholder="Overnight Stay" title="Overnight Stay"  value="<?php echo $row_program['stay']; ?>"></td>
                          <td class='col-md-2'><select id="meal_plan<?php echo $offset; ?>" title="Meal Plan" name="meal_plan" class="form-control mg_bt_10">
                                  <option value="<?php echo $row_program['meal_plan']; ?>"><?php echo $row_program['meal_plan']; ?></option>
                                  <?php get_mealplan_dropdown(); ?>
                                     </select></td>
                          <td><input style="display:none" type="text" name="package_id_n" value="<?php echo $row_tours['package_id']; ?>"></td>
                        </tr>
                    <?php $offset++;
                     } ?>
                  </table>
