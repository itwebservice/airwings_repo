		...
		$.ajax({
	      ...
	      success:function(result){
	      	...
	      	for(var i=0; i<train_arr.length; i++){
	      		...
	      		row.cells[6].childNodes[0].value = train_arr[i]['arrival_date'];
	      		row.cells[5].childNodes[0].value = train_arr[i]['departure_date'];	      		
		...
		//Flight Info
	      ...
	      success:function(result){
	      	...
	      	for(var i=0; i<plane_arr.length; i++){
	      		...
	      		var row = table.rows[i];
				row.cells[2].childNodes[0].value = plane_arr[i]['from_city'];
	      		$(row.cells[3].childNodes[0]).html('<option value="'+plane_arr[i]['from_location']+'">'+plane_arr[i]['from_location']+'</option>');

	      		row.cells[4].childNodes[0].value = plane_arr[i]['to_city'];
	      		$(row.cells[5].childNodes[0]).html('<option value="'+plane_arr[i]['to_location']+'">'+plane_arr[i]['to_location']+'</option>');

	      		row.cells[6].childNodes[0].value = plane_arr[i]['airline_name'];
	      		row.cells[7].childNodes[0].value = plane_arr[i]['class'];
	      		row.cells[8].childNodes[0].value = plane_arr[i]['dapart_time'];
	      		row.cells[9].childNodes[0].value = plane_arr[i]['arraval_time'];	
	      ...
	      success:function(result){
			...
			var total_child = $("#total_children").val();
	      	var child_cost = total_child * cost_arr.children_cost;
	      	$("#children_cost").val(parseFloat(child_cost));
			var total_infant = $("#total_infant").val();
	      	...
	      	var total_cost = parseFloat(adult_cost) + parseFloat(children_cost) + parseFloat(infant_cost) + parseFloat(with_bed_cost);     	 
