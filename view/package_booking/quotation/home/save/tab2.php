...
$('#frm_tab2').validate({
	...
	submitHandler:function(form)
	{
		...
		var total_pax = 0;
		var total_rooms = 0; var total_rooms1 = 0;

		total_pax = parseFloat(total_adult) + parseFloat(total_children);
		if(total_pax == 1){
  	  		total_rooms1 = 1;
			extra_bed = 0;
		}
		else{
			total_rooms1 = (total_pax/2);
			extra_bed = (total_pax%2);
		}
  	  	total_rooms= Math.floor(total_rooms1);
	      ...
	      success:function(result){
					...
					if(table.rows.length == 1){
						...
						var extra_bed_cost = 0 ;
						for(var i=0; i<hotel_arr.length; i++){
							var row = table.rows[i]; 
							row.cells[2].childNodes[0].value = hotel_arr[i]['city_id'];
							...
							row.cells[5].childNodes[0].value = hotel_arr[i]['total_days'];
							row.cells[6].childNodes[0].value = total_rooms;
							row.cells[7].childNodes[0].value = extra_bed;
							
							extra_bed_cost = parseFloat(hotel_arr[i]['extra_bed_cost']) * parseFloat(extra_bed);
							
							row.cells[8].childNodes[0].value = hotel_arr[i]['package_name'];
							row.cells[9].childNodes[0].value = hotel_arr[i]['hotel_cost'];
							row.cells[10].childNodes[0].value = hotel_arr[i]['package_id'];
	      ...
	      success:function(result){
			...
			var transport_arr = JSON.parse(result);
	      	...
	      	for(var i=0; i<transport_arr.length; i++){
	      		...
	      		row.cells[5].childNodes[0].value = transport_arr[i]['package_name'];
	      		row.cells[6].childNodes[0].value = transport_arr[i]['transport_cost'];
	      		row.cells[7].childNodes[0].value = transport_arr[i]['package_id'];
	      	}
	      }
	      });
	      ...
	      success:function(result){
	        ...
	        for(var i=0; i<costing_arr.length; i++){
	      		var row = table.rows[i];
	      		row.cells[11].childNodes[0].value = costing_arr[i]['package_name'];
	      		row.cells[12].childNodes[0].value = costing_arr[i]['package_id'];
	      	}
	        ...
	        for(var i=0; i<costing_arr.length; i++){
	      		...
	      		row.cells[1].childNodes[0].value = costing_arr[i]['adult_cost'];
	      		row.cells[2].childNodes[0].value = costing_arr[i]['child_cost'];
	      		row.cells[3].childNodes[0].value = costing_arr[i]['infant_cost'];
	      		row.cells[4].childNodes[0].value = costing_arr[i]['child_with'];
	      		row.cells[5].childNodes[0].value = costing_arr[i]['child_without'];
	      		row.cells[6].childNodes[0].value = costing_arr[i]['extra_bed'];
	      		row.cells[7].childNodes[0].value = costing_arr[i]['package_id'];
	      	}
	      }
	      });
