			...
			<div class="col-md-12 app_accordion">
			    <div class="panel-group main_block" id="accordion" role="tablist" aria-multiselectable="true">
	                <div class="panel-body">
	                    ...
	                    <table style="width:100%" id="dynamic_table_list_update" name="dynamic_table_list_update" class="table mg_bt_0 table-bordered">
	                        ...
	                        <?php
		                      ...
		                      ?>
		                        <tr>
			                          <td width="27px;"><input class="css-checkbox mg_bt_10" id="chk_program<?= $offset ?>" type="checkbox" checked ><label class="css-label" for="chk_program<?= $offset ?>"> </label></td>
			                          <td width="50px;"><input maxlength="15" value="<?= $offset ?>" type="text" name="username" placeholder="Sr. No." class="form-control mg_bt_10" disabled /></td>
			                          <td class='col-md-3 no-pad'><input type="text" id="special_attaraction<?php echo $offset; ?>" onchange="validate_spaces(this.id);validate_spattration(this.id);" name="special_attaraction" class="form-control mg_bt_10" placeholder="Special Attraction" title="Special Attraction"  value="<?php echo $row_program['attraction']; ?>"></td>
			                          <td class='col-md-6 no-pad' style="max-width: 594px;overflow: hidden;"><textarea id="day_program<?php echo $offset; ?>" name="day_program" class="form-control mg_bt_10" title="Day-wise Program" rows="3" placeholder="*Day-wise Program" onchange="validate_spaces(this.id);validate_dayprogram(this.id);" value="<?php echo $row_program['day_wise_program']; ?>"><?php echo $row_program['day_wise_program']; ?></textarea></td>
			                          <td class='col-md-2 no-pad' style='width:100px'><input type="text" id="overnight_stay<?php echo $offset; ?>" name="overnight_stay" onchange="validate_spaces(this.id);validate_onstay(this.id);" class="form-control mg_bt_10" placeholder="Overnight Stay" title="Overnight Stay"  value="<?php echo $row_program['stay']; ?>"></td>
			                          <td class='col-md-1 no-pad'><select id="meal_plan<?php echo $offset; ?>" title="Meal Plan" name="meal_plan" class="form-control mg_bt_10">
			                                  <option value="<?php echo $row_program['meal_plan']; ?>"><?php echo $row_program['meal_plan']; ?></option>
			                                  <?php get_mealplan_dropdown(); ?>
			                                     </select></td>
			                          <td><input type="hidden" name="package_id_n" value="<?php echo $row_program['id']; ?>"></td>
			                    </tr>
						      <?php } ?>
							</table>
							<div class="row mg_tp_20">
								<div class="col-md-6">  
								  ...
								  <textarea class="feature_editor" id="inclusions1" name="inclusions1" placeholder="Inclusions" title="Inclusions" rows="4"><?php echo $sq_quotation['inclusions']; ?></textarea>
								</div>
								<div class="col-md-6">  
								  <h3 class="editor_title">Exclusions</h3>
								  <textarea class="feature_editor" id="exclusions1" name="exclusions1" placeholder="Exclusions" title="Exclusions" rows="4"><?php echo $sq_quotation['exclusions']; ?></textarea>
								</div>                
							</div>                    
		                </div>    
		            	</div>
