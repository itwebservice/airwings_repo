        ...
        <table id="tbl_dynamic_cruise_quotation" name="tbl_dynamic_cruise_quotation" class="table mg_bt_0 table-bordered mg_bt_10">
        	...
        	if($sq_cruise_count==0){
				...
				<tr>
		            ...
		            <td><input type="text" id="cruise_departure_date" name="cruise_departure_date" placeholder="Departure Date Time" title="Departure Date Time" class="app_datetimepicker" onchange="get_to_datetime(this.id,'cruise_arrival_date')" value="<?= date('d-m-Y H:i:s') ?>"></td>
		            ...
		            <script>
		            	$('#cruise_arrival_date, #cruise_departure_date').datetimepicker({ timepicker:true, format:'d-m-Y H:i:s' });
		            </script>          
        		<?php
        	    } 
        		else{
        		$count = 0;
        		$sq_q_cruise = mysql_query("select * from package_tour_quotation_cruise_entries where quotation_id='$quotation_id'");
        		while($row_q_cruise = mysql_fetch_assoc($sq_q_cruise)){
        			$count++;
        			?>
					<tr>
		                <td><input class="css-checkbox" id="chk_cruise<?= $count ?>_1" type="checkbox" checked disabled><label class="css-label" for="chk_cruise<?= $count ?>_1"> <label></td>
		                <td><input maxlength="15" value="<?= $count ?>" type="text" name="username" placeholder="Sr. No." class="form-control" disabled /></td>
			            <td><input type="text" id="cruise_departure_date<?= $count ?>_u" name="cruise_departure_date<?= $count ?>_u" placeholder="Departure Datetime" title="Departure Datetime" class="app_datetimepicker" onchange="get_to_datetime(this.id,'cruise_arrival_date<?= $count ?>_u')" value="<?= get_datetime_user($row_q_cruise['dept_datetime']) ?>"></td>
			            <td><input type="text" id="cruise_arrival_date<?= $count ?>_u" name="cruise_arrival_date<?= $count ?>_u" placeholder="Arrival Datetime" title="Arrival Datetime" class="app_datetimepicker" value="<?= get_datetime_user($row_q_cruise['arrival_datetime']) ?>"></td>
		            ...
		            <script>
		            	$('#cruise_arrival_date<?= $count ?>_u, #cruise_departure_date<?= $count ?>_u').datetimepicker({ timepicker:true, format:'d-m-Y H:i:s' });
		            </script>
        			<?php
        		}
        	}
