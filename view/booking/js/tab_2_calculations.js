...
function calculate_total_train_expense(){
  ...
  if(train_expense==""){ train_expense=0; }
  if(service_charge==""){ service_charge=0; }
  if(train_service_tax==""){ train_service_tax=0; }
...
function calculate_plane_expense(id)
{
  ...
  for(var i=0; i<rowCount; i++)
  {
    ...
    if(row.cells[0].childNodes[0].checked == true)
    {
    var value1 = row.cells[9].childNodes[0].value;    
...
function calculate_total_plane_expense()
{
  ...
  if(plane_expense==""){ plane_expense = 0; }
  if(service_charge==""){ service_charge = 0; }
  if(plane_service_tax==""){ plane_service_tax = 0; }
...
function calculate_total_cruise_expense(){
  ...
  if(cruise_expense==""){ cruise_expense = 0; }
  if(service_charge==""){ service_charge = 0; }
  if(cruise_service_tax==""){ cruise_service_tax = 0; }
