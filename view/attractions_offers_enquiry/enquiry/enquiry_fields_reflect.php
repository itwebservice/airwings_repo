...
if($enquiry_type=="Group Booking" || $enquiry_type=="Package Booking"){
	...
	if(isset($_POST['enquiry_id'])){
		...
		foreach($enquiry_content_arr1 as $enquiry_content_arr2){
			...
			if($enquiry_content_arr2['name']=="cmb_meal_plan"){ $sq_c['cmb_meal_plan'] = $enquiry_content_arr2['value']; }
		}
	}
	else{
		$sq_c['tour_name'] = $sq_c['budget'] = $sq_c['total_members'] = $sq_c['total_adult'] = $sq_c['total_children'] = $sq_c['total_infant'] = $sq_c['children_without_bed'] = $sq_c['children_with_bed'] = $sq_c['travel_from_date'] = $sq_c['travel_to_date']  = $sq_c['hotel_type']=$sq_c['cmb_meal_plan'] = "";
	}
...
<div class="row">
	...
	<div class="col-md-4 col-sm-6 mg_bt_10">
		<select title="Meal Plan" id="cmb_meal_plan1" name="cmb_meal_plan" title="Meal Plan" style="width:110px;">
		<?php
		if($sq_c['cmb_meal_plan']!=""){
					?>
					<option value="<?= $sq_c['cmb_meal_plan'] ?>"><?= $sq_c['cmb_meal_plan'] ?></option>
					<?php
				get_mealplan_dropdown(); 
				 } ?>
