	...
	<ul class="no-pad">
		<?php
		 ...
		 if($role!='Admin' && $role!='Branch Admin'){
		  $sq_enquiry = mysql_query("select * from enquiry_master where status='' and assigned_emp_id='$emp_id' order by enquiry_id desc limit 5");
        ...
        $query ="select * from enquiry_master as en left join enquiry_master_entries as ef on en.enquiry_id=ef.enquiry_id where en.status!='Disabled'";
			...
			$query .=" order by ef.created_at desc limit 5";
