...
$objPHPExcel->getActiveSheet()->getStyle('A'.$row_count.':W'.$row_count)->applyFromArray($header_style_Array);
$objPHPExcel->getActiveSheet()->getStyle('A'.$row_count.':W'.$row_count)->applyFromArray($borderArray);

$row_count++;
$count = 0;

    // $sq_salary = "select e.emp_id ,e.first_name , e.last_name , e.dob , e.gender , e.email_id, e.mobile_no
    //     , e.age , e.target , e.incentive,e.incentive_per , e.mobile_no2 , e.date_of_join , e.username , e.password ,e.uan_code, e.app_smtp_status , e.app_smtp_host , e.app_smtp_port , e.app_smtp_password ,e.app_smtp_method
    //     , l.location_name , r.role_name , b.branch_name , b.address1 , b.address2 , b.city from emp_master as e left join role_master as r on e.role_id=r.role_id left join locations as l on e.location_id=l.location_id left join branches as b on e.branch_id=b.branch_id where 1";
    // if($role_filter!=''){
    //     $sq_salary .= " and r.role_name = '$role_filter'";
    // } 

    // if($location_id_filter!=''){
    //     $sq_salary .= " and e.location_id = '$location_id_filter'";
    // }

    // if($branch_id_filter!=''){
    //     $sq_salary .= " and e.branch_id = '$branch_id_filter'";
    // }
    ...
    ->setCellValue('J'.$row_count, $sq_sal['location_name'])
    ->setCellValue('K'.$row_count, $sq_sal['branch_name'])
    ->setCellValue('L'.$row_count, $sq_sal['address1'].' '.$sq_sal['address2'].' '.$sq_sal['city'])
    ->setCellValue('M'.$row_count, $sq_sal['username'])
    ->setCellValue('N'.$row_count, $sq_sal['password'])
    ...
    $objPHPExcel->getActiveSheet()->getStyle('A'.$row_count.':W'.$row_count)->applyFromArray($content_style_Array);
    $objPHPExcel->getActiveSheet()->getStyle('A'.$row_count.':W'.$row_count)->applyFromArray($borderArray);    
