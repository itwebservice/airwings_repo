...
$payment_dates = ($sq_customer['payment_date'] == '0000-00-00' || $sq_customer['payment_date'] == '1970-01-01') ? '' : 'readonly';
        ...
        <input type="hidden" id="register_id" name="register_id" value="<?= $register_id ?>">
          <div class="row mg_bt_10">
            <div class="col-sm-3 mg_bt_10_sm_xs">
              <select name="approve_status1" id="approve_status1" title="Approval Status" onchange="change_fields_status(this.id)">
            ...
            <div class="col-md-4 col-sm-6 text-left">          
              <div class="div-upload">
                ...
                <ul id="files" ></ul>
                <input type="hidden" id="photo_upload_url" name="photo_upload_url" value="<?= $sq_customer['agreement_url'] ?>">
              </div>
            ...
            <div class="row mg_tp_10">
              ...
              <?php if($sq_customer['approval_status'] == 'Approved'){ ?>
                <div class="col-sm-3 mg_bt_10_sm_xs">
                  <input type="text" id="deposit" name="deposit" placeholder="Amount" title="Deposit Amount" value="<?= $sq_customer['deposite'] ?>" onchange="validate_balance(this.id);">
                </div>
                <div class="col-md-3">
                  <input type="text" id="payment_date" name="payment_date" placeholder="*Date" title="Date" value="<?= get_date_user($sq_customer['payment_date']) ?>" onchange="check_valid_date(this.id)" <?= $payment_dates ?>>
                </div>
                <div class="col-md-3">
                  <select name="payment_mode" id="payment_mode" title="Mode" onchange="payment_master_toggles(this.id, 'bank_name', 'transaction_id', 'bank_id')" <?= $payment_modes ?>>
  ...
  $(function(){
	$('#frm_customer_update').validate({
	  ...
	  submitHandler:function(form){
        ...
        var agreement_url = $('#photo_upload_url').val();
        var approve_status = $('#approve_status1').val();
        //Deposit
