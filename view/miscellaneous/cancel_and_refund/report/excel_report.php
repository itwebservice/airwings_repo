...
while($row_refund_entry = mysql_fetch_assoc($sq_refund_entries)){
  ...
  $objPHPExcel->getActiveSheet()->getStyle('B'.$row_count.':I'.$row_count)->applyFromArray($content_style_Array);
    ...
    $objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('F'.$row_count, 'Refund Amount : '.number_format($total_refund,2))
        ...
        ->setCellValue('I'.$row_count, 'Total Paid : '.number_format($sq_paid_amount,2));
...
header('Content-Disposition: attachment;filename="Miscellaneous Booking Refund('.date('d-m-Y H:i:s').').xls"');
