...
$financial_year_id = $_SESSION['financial_year_id'];
        ...
        while($row_group_bookings = mysql_fetch_assoc($sq_group_bookings)){
          ...
          $pass_count = mysql_num_rows(mysql_query("select * from  travelers_details where traveler_id='$row_group_bookings[id]'"));
          $cancelpass_count = mysql_num_rows(mysql_query("select * from  travelers_details where traveler_id='$row_group_bookings[id]' and status='Cancel'"));
  ...
  <table class="table table-hover" style="margin: 0 !important;border: 0;padding: 0 !important;">
    ...
    <?php 
      foreach($booking_array as $booking_array_item){
        ...
        if($other_data_arr['status']=='Cancel'){ $row_bg = "danger"; }
    ...
    ?>
        <tr class="<?= $row_bg ?>">
          ...
          <td><?= $other_data_arr['tour_cost_total'] ?></td>
          <td><?= $incentive_total ?></td>
        </tr>
        </tbody>
        <?php } ?>
  </table>
