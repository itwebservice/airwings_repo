...
if($booking_id!=""){
  ...
  $year =$yr[0];
  $invoice_id = get_group_booking_id($booking_id,$year);
}
...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('B7', 'Receipt For')
            ->setCellValue('C7', $payment_str)
            ->setCellValue('B8', 'Payment Mode')
            ->setCellValue('C8', $pay_mode)
            ->setCellValue('B9', 'From-To-Date')
            ->setCellValue('C9', $date_str)
            ->setCellValue('B10', 'Customer Type')
            ->setCellValue('C10', $cust_type)
            ->setCellValue('B11', 'Company Name')
            ->setCellValue('C11', $company_name);
...
$objPHPExcel->getActiveSheet()->getStyle('B11:C11')->applyFromArray($header_style_Array);
$objPHPExcel->getActiveSheet()->getStyle('B11:C11')->applyFromArray($borderArray);   
...
$objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('G'.$row_count, "Receipt For")
...
while($row_payment = mysql_fetch_assoc($sq_payment)){
  if($row_payment['amount']!=0){
      ...
      $group = get_date_user($sq_group['from_date']).' to '.get_date_user($sq_group['from_date']);
